  integer, parameter :: n=256*2,nx=n,ny=2,nz=256*2,neq=4
  real, dimension(neq,nx,ny,nz) :: u
  character(100),parameter :: dir='data/r1.2375_022'!!my test, wrong direction
print*,'writing in: ',trim(dir)
  !  call initriemann
  call initshock4(.true.)
!  open(10,file='u0.dat',access='stream')
!  write(10) u(1,:,:,:)
!  stop
  cfl=0.35
  open(10,file=trim(dir)//'/u0.dat',access='stream')
  open(18,file=trim(dir)//'/pk.dat',access='stream')
  open(11,file=trim(dir)//'/udiv.dat',access='stream')
  open(12,file=trim(dir)//'/ucrl.dat',access='stream')
  open(13,file=trim(dir)//'/ucrly.dat',access='stream')
  t=0
  do it=1,500
     do i=1,16!n/40+1
        dt=timestep()
...
!        call viscosity
        ...
        call sweepy
        call sweepx
        call initshock4(.false.)
        t=t+dt
     end do
     write(*,*) t,it
     open(110,file=trim(dir)//'/icount.dat')
     rewind(110)
     write(110,*) it
     close(110)
     write(10) u(1,:,ny/2,:)
...
contains
  ...
  subroutine initshock4(firsttime)
    ...
    real x,y,vx,vy,u0,gamma,phi,beta,pi,alpha,eps
    integer i,j
    integer, parameter :: nbuf=6*2*2
